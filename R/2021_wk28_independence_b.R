...
p1 <- ggplot() +
  ...
  # annotate and arrow
  geom_text(aes(x=1, y=0.8, label=str_wrap('Number of countries colonized', 20)), family=font[5], size=8, color=label_pal[5], lineheight=0.3) +
  geom_curve(aes(x=0.9, xend=0.6, y=0.1, yend=0.5), size=0.5, curvature = -0.5, arrow = arrow(length = unit(0.1, "inches")), lineend = "round", alpha=0.3, color=label_pal[5]) +
  
  # title
  geom_richtext(data=title, aes(x=7.5, y=1.8, label=str_squish(label)), hjust=0.5, vjust=0.5, family=font[4], lineheight=0.5,  color=NA, fill=NA, size=13) +
...
# merge plot
plot <- p1 + p2 + plot_layout(heights = c(3,5))

# save plot
ggsave('2021_wk28_independence_b.png', plot=plot, type = 'cairo', width = 9, height = 7, dpi = 300, units = "in", bg = bg_pal[1])
